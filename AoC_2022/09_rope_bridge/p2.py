...
def main():
  knots_pos: list[TCoord] = [(0, 0)] * 10
  tail_coords: set[TCoord] = set([knots_pos[-1]])
  ...
  for dir_str, dist in instructions:
    for _ in range(dist):
      knots_pos[0] = move_head(knots_pos[0], dir_str)
      for i in range(1, len(knots_pos)):
        knots_pos[i] = move_tail(knots_pos[i], knots_pos[i-1])
      tail_coords.add(knots_pos[-1])
      # print_board(rows=5, cols=6, head_pos=head_pos, tail_pos=tail_pos)
