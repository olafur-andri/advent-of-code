...
def find_non_main_loop_coords(pipe_grid: TPipeGrid, nr_rows: int, nr_columns: int) -> tuple[TCoord, ...]:
    ...

def construct_placement_dict(loop: tuple[TCoord, ...], pipe_grid: TPipeGrid, nr_rows: int, nr_columns: int) -> MappingProxyType[TCoord, Placement]:
    non_main_loop_coords = find_non_main_loop_coords(loop, nr_rows, nr_columns)
    placement_dict = { empty_coord: Placement.UNKNOWN for empty_coord in non_main_loop_coords }

    # TODO
    # while True:
    #     unknown_empty_coord = 

    return MappingProxyType(placement_dict)

def fix_starting_tile(pipe_grid: TPipeGrid, nr_rows: int, nr_columns: int, main_loop: tuple[TCoord, ...]):
    starting_coord = find_starting_coord(pipe_grid, nr_rows, nr_columns)

    neighbor_coords = get_connected_neighbor_coords(starting_coord, pipe_grid, nr_rows, nr_columns)
    main_loop_neighbor_coords = [coord for coord in neighbor_coords if coord in main_loop]
    neighbor_directions = [Direction.from_to(starting_coord, neighbor_coord) for neighbor_coord in main_loop_neighbor_coords]

    return get_pipe_from_directions(neighbor_directions)

def get_area_within_loop(loop: tuple[TCoord, ...], pipe_grid: TPipeGrid, nr_rows: int, nr_columns: int) -> int:
    placement_dict = construct_placement_dict(loop, pipe_grid, nr_rows, nr_columns)
    return len([coord for coord, placement in placement_dict.items() if placement == Placement.INSIDE])
...
def main():
    ...
    fix_starting_tile(pipe_grid, nr_rows, nr_columns, main_loop)
    area = get_area_within_loop(main_loop, pipe_grid, nr_rows, nr_columns)
